...
\newline
\newline
Do you need {\Large{remote view}} on Linux systems??\newline
\newline
\newline
};
% ------------------------------------------------------------------------------------------------------------------------------
...
\begin{scriptsize}
\begin{lstlisting}
1. (remote) enable port in iptables -> iptables -A INPUT -p tcp --dport 5900 -j ACCEPT
2. (remote) start vncserver -> x11vnc -forever -usepw -display :0 &
3. (local)  connect -> vncviewer remote\_ip:0 or on MacOS open vnc://remote
\end{lstlisting}
\end{scriptsize}
\newline
\newline
