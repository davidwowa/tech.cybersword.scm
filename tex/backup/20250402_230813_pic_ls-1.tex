...
\newline
\newline
You’ve found multiple {\Large{similar files}}, no GUI involved, and now you want to {\Large{automate further actions}} with a script.\newline
\newline
\newline
};
% ------------------------------------------------------------------------------------------------------------------------------
...
Use {\large{ls -1 > script.sh}} command.\newline
\newline
{\large{-1}} - list entries in folder \newline
{\large{> script.sh}} - bash script \newline
\newline
Then prepend your tool or command to each line, and don’t forget to add:\newline
\texttt{\#!/bin/bash}\newline
on top.
\newline
};
