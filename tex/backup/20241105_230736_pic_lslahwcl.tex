% Voraussetzungen: PDF wird erstellt und später in PNG konvertiert (z.B. mit ImageMagick)
...
\begin{document}
\begin{tikzpicture}
    % Bildgröße definieren (1080x1080pt)
    ...
    \node[red, anchor=north west, font=\ttfamily\fontsize{8}{9}\selectfont, text width=10.6cm, align=center] at (0.1,10.25) {
            During penetration test you find a {\huge{large directory}} with a {\huge{lot of files}}, you want the {\huge{number/counter}} of files.
%         \begin{lstlisting}
% # Hier kann der Text für das Problem stehen
% echo "Dies ist ein Beispieltext für das Problem."
%         \end{lstlisting}
    };
    ...
    \node[green, anchor=north west, font=\ttfamily\fontsize{8}{9}\selectfont, text width=10.6cm, align=center] at (0.1,5.25) {
        Use the {\huge{\texttt{ls}}} command with the {\huge\texttt{-l}} option to list the files in the directory and count with {\huge\texttt{wc -l}} the number of lines.\newline
        \newline
        \newline
        Command: {\huge\texttt{ls -l | wc -l}}
%         \begin{lstlisting}
% %         \begin{lstlisting}
% % # Hier kann der Text für die Lösung stehen
% % echo "Dies ist ein Beispieltext für die Lösung."
% %         \end{lstlisting}
    };
